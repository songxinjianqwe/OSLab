...
PRIVATE void getQueryStr(int begin, int end);
...
PUBLIC void in_process(u32 key) {
    ...
    if (!(key & FLAG_EXT)) {
        ...
        if (!NORMAL_MODE && query_mode_start_pos == 0) {
        ...
        switch (raw_key) {
            case ENTER:
                ...
                if (!NORMAL_MODE && has_entered_in_query_mode == 0) {
                    printSpecialChar('\n');
                    //do query
                    getQueryStr(query_mode_start_pos, disp_pos);
            ...
            case ESC:
                if (NORMAL_MODE) {
                    ...
                    for (int i = 0; i < (curr_pos - query_mode_start_pos) / 2; ++i) {
                        printSpecialChar('\b');
                    }
                    has_entered_in_query_mode = 0;
                    query_mode_start_pos = 0;
...
PRIVATE void getQueryStr(int begin, int end) {
    read_display_memory(begin, end, queryStr);
    disp_str(queryStr);
}
